...
pair<int,int> getmax (int l, int r, int i = 1, int tl = 0, int tr = n - 1, int pl = 0)
{
	pl += v[i];
	if (tl == l && tr == r)
	...
	pair<int,int>a,b;a=getmax(l, m, i + i, tl, m, pl);b=getmax(m + 1, r, i + i + 1, m + 1, tr, pl);
	if(a >= b )
		return a;
	return b;
}
